...
def sample_neg(mat, test_ratio=0.1, train_pos=None, test_pos=None, max_train_num=None):
    ...
    mat_triu = ssp.triu(mat, k=1)
    # sample positive links for train/test
    row, col, _ = ssp.find(mat_triu)
    ...
    while len(neg[0]) < train_num + test_num: # neg_ratio is 0.5
...
def entity2vid(link, match):
    num = len(link[0])
    for i in range(num): 
        link[0][i]=match[np.argwhere(match[:,1] == link[0][i])[0]][0][0]
        link[1][i]=match[np.argwhere(match[:,1] == link[1][i])[0]][0][0]
    return link


def verify_sample(net, train_pos, train_neg, test_pos, test_neg):
    train_num, test_num = len(train_pos[0]), len(test_pos[0])
    train_pos_assert = np.count_nonzero(net[train_pos]==np.ones(train_num))==train_num
    train_neg_assert = np.count_nonzero(net[train_neg]==np.zeros(train_num))==train_num
    test_pos_assert = np.count_nonzero(net[test_pos]==np.ones(test_num))==test_num
    test_neg_assert = np.count_nonzero(net[test_neg]==np.zeros(test_num))==test_num
    print('verifying that pos and neg links are correct')
    print('train_pos == 1:', train_pos_assert)
    print('train_neg == 0:', train_neg_assert)
    print('test_pos  == 1:', test_pos_assert)
    print('test_neg  == 0:', test_neg_assert)
    assert(train_pos_assert and train_neg_assert and test_pos_assert and test_neg_assert)
...
def links2subgraphs(A, train_pos, train_neg, test_pos, test_neg, h=1, max_nodes_per_hop=None, embeddings=None, attributes=None, parallel=False):
    ...
    def helper(A, links, g_label, parallel=parallel):
        if not parallel:
            ...
            for i, j in pbar:
                nodes, g, n_labels, n_embeds, n_attrs = subgraph_extraction_labeling((i, j), A, h, max_nodes_per_hop, embeddings, attributes)
                max_n_label['value'] = max(max(n_labels), max_n_label['value'])
                g_list.append(GNNGraph(nodes, g, g_label, n_labels, n_embeds, n_attrs))
            return g_list
        else:
            # parallel extraction code
            start = time.time()
            pool = mp.Pool(mp.cpu_count())
            results = pool.map_async(parallel_worker, [((i, j), A, h, max_nodes_per_hop, embeddings, attributes) for i, j in zip(links[0], links[1])])
            ...
            pool.close()
            pbar.close()
            g_list = [GNNGraph(nodes, g, g_label, n_labels, n_embeds, n_attrs) for nodes, g, n_labels, n_embeds, n_attrs in results]
...
def subgraph_extraction_labeling(ind, A, h=1, max_nodes_per_hop=None, embeddings=None, attributes=None):
    ...
    # get node features
    embeds, attrs = None, None
    if embeddings is not None:
        embeds = embeddings[nodes]
    if attributes is not None:
        attrs = attributes[nodes]
    # construct nx graph
    g = nx.from_scipy_sparse_matrix(subgraph)
    # remove link between target nodes
    if g.has_edge(0, 1):
        g.remove_edge(0, 1)
    return nodes, g, labels.tolist(), embeds, attrs
